...
DEFAULT_SPECTROGRAM_PATH = PROJECT_ROOT / "visualizations" / "latest_realtime_spectrogram.png"
...
def update_live_plot(plt, axes, mel_db, linear_db, prob):
    ...
    axes[1].set_xlabel(f"Fake probability over {DURATION:.1f}s: {prob:.4f}")
    plt.pause(0.001)
...
def run_realtime_demo(args):
    ...
    if not model_path.exists():
        raise FileNotFoundError(
            f"Trained model not found: {model_path}. Train first with main.py."
        )
    ...
    print("Realtime detector started. Press Ctrl+C to stop.")
    print(f"Using model: {model_path}")
    print(
        f"Decision window: {DURATION:.1f}s, update interval: {args.interval:.1f}s"
    )
    ...
    try:
        while True:
            ...
            print(
                ...
                f"fake={prob:.4f} | window={DURATION:.1f}s | {label}"
            )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run realtime audio deepfake detection from a microphone."
    )
    parser.add_argument(
        "--model",
        default=str(DEFAULT_MODEL_PATH),
        help="Path to the trained dual-input .keras model.",
    )
    parser.add_argument(
        "--threshold",
        type=float,
